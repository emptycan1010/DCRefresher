      ...
      <div class="refresher-preview-contents" v-if="!frame.error" :class="{collapse: frame.collapse}">
        ...
        <div class="refresher-preview-comments" v-if="frame.data.comments && frame.data.comments.comments">
          <transition-group name="refresher-slide-up" appear @before-enter="beforeEnter" @after-enter="afterEnter">
            <Comment v-for="(comment, i) in frame.data.comments.comments" :index="i + 1" :postUser="frame.data.postUserId" :comment="comment" :key="'cmt_' + comment.no" :delete="frame.functions.deleteComment"></Comment>
          </transition-group>
        </div>
